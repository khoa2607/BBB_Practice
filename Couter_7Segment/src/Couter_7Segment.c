 ...
 * A 				 P8_18				gpio2[1]		gpio65 //change from p8_7 to p8_18, [66 to 65]
...
//read value of GPIO // to return High Or Low ,
int gpio_read(uint32_t gpio_num){

	int fd;
	char buf[100];
	uint8_t value = 0;

	//concatenate the path AGAIN
	snprintf(buf,sizeof(buf),PATH"/gpio%d/value",gpio_num);

	//open the file
	fd = open(buf,O_WRONLY);

	if (fd<0){
		perror("path is wrong to read value of gpio");
		return fd;
	}
